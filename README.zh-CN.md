...
- **面向日常使用。** 内置 Tabby 主题，可调字体、行距、光标和背景透明度，并记住窗口布局。
...
- SSH 配置选择器现在会跟随当前终端主题，并以不透明、高对比度的搜索框和选中状态保证可读性。
- 顶栏、侧边栏和空工作区会跟随当前终端主题，并保持界面文字可读。
- 背景自定义简化为单一透明度控制。
- 设置可通过 Escape 或 Cmd+W 关闭；没有标签页时不再显示标签栏。
